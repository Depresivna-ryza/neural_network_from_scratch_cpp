...
struct Tensor {
    ...
    double get(std::vector<size_t> index) const {
        return data[get_index(index)];
    }

    void set(std::vector<size_t> index, double value) {
...
std::string to_string() const {
    return "Tensor{" + to_string_recursive(0, 0) + "}";
}
...
std::string to_string_recursive(int dim, int index) const {
    ...
    for (int i = 0; i < shape[dim]; ++i) {
        ...
        result += "\n" + std::string(dim + 1, ' ') + to_string_recursive(dim + 1, index * shape[dim] + i);
    }
    result += "\n" + std::string(dim, ' ') + "]";
    return result;
}
